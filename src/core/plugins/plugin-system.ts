...
export class PluginSystem {
  ...
  public onServerStart(): void {
    this.safeInvoke((plugin) => plugin.onServerStart?.());
  }

  public onServerStop(): void {
    this.safeInvoke((plugin) => plugin.onServerStop?.());
  }
  ...
  public onRoomError(room: BaseRoom, context: string, error: unknown): void {
    this.safeInvoke((plugin) => plugin.onRoomError?.(room, context, error));
  }

  public onSlowTick(room: BaseRoom, event: SlowTickEvent): void {
    this.safeInvoke((plugin) => plugin.onSlowTick?.(room, event));
  }
