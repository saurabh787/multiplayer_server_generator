...
async function main(): Promise<void> {
  log("[diag-phase]", { step: "zombie-room-check", durationSec: 180 });
  const short = await runSoak({
    label: "zombie-check-3m",
    durationMs: 180_000,
    clients: 30,
    port: 3350,
    metricsPort: 3351
  ...
  log("[diag-phase]", { step: "full-soak-10m", durationSec: 600 });
  ...
  const overallPass = short.pass && full.pass;
  log("[diag-overall]", { overallPass });
  if (!overallPass) {
    process.exitCode = 1;
  }
}
